...
pub struct KmeansTestNode { // <--- Wenn man eine neue Node anlegt, einfach alles kopieren und hier den Namen ändern
    #[output]
...
impl Node for KmeansTestNode {
    ...
    fn on_update(&mut self) -> Result<(), UpdateError> {
        ...
        if let Ok(node_data) = self.input.next() {
            println!("JW-Debug: KMeansTestNode has received: {}.", node_data);
...
fn input_output_test() -> Result<(), UpdateError> {
    ...
    let mut and: KmeansTestNode<> = KmeansTestNode::new(Some(&change_observer));
